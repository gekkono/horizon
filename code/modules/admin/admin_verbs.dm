...
/client/proc/test_cardpack_distribution()
	...
	if(!SStrading_card_game.loaded)
		message_admins("The card subsystem is not currently loaded")
		return
	var/pack = tgui_input_list("Which pack should we test?", "You fucked it didn't you", sortList(SStrading_card_game.card_packs))
...
/client/proc/give_spell(mob/spell_recipient in GLOB.mob_list)
	...
	var/spell_desc = tgui_input_list(usr, "Choose the spell to give to that guy", "ABRAKADABRA", sortList(spell_list))
	if(!spell_desc)
		return
...
/client/proc/remove_spell(mob/removal_target in GLOB.mob_list)
	...
	var/obj/effect/proc_holder/spell/removed_spell = tgui_input_list(usr, "Choose the spell to remove", "NO ABRAKADABRA", sortList(target_spell_list))
	if(removed_spell)
...
/client/proc/give_disease(mob/living/T in GLOB.mob_living_list)
	...
	if(!istype(T))
		...
		return
	var/datum/disease/D = tgui_input_list(usr, "Choose the disease to give to that guy", "ACHOO", sortList(SSdisease.diseases, /proc/cmp_typepaths_asc))
	if(!D)
		return
