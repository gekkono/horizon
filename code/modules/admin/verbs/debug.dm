...
/client/proc/cmd_debug_del_all(object as text)
	...
	if(matches.len==0)
		return
	var/hsbitem = tgui_input_list(usr, "Choose an object to delete.", "Delete:", sortList(matches))
	if(hsbitem)
...
/client/proc/robust_dress_shop()
	...
	if (dresscode == "As Job...")
		...
		dresscode = tgui_input_list(usr, "Select job equipment", "Robust quick dress shop", sortList(job_outfits))
...
/client/proc/jump_to_ruin()
	...
	var/ruinname = tgui_input_list(src, "Select ruin", "Jump to Ruin", sortList(names))
...
/client/proc/place_ruin()
	...
	var/ruinname = tgui_input_list(src, "Select ruin", "Spawn Ruin", sortList(names))
