...
/mob/living/carbon/vv_do_topic(list/href_list)
	...
	if(href_list[VV_HK_MODIFY_BODYPART])
		...
		if(edit_action == "remove" || edit_action == "augment")
			...
			for(var/obj/item/bodypart/B in bodyparts)
				limb_list -= B.body_zone
		var/result = tgui_input_list(usr, "Please choose which body part to [edit_action]","[capitalize(edit_action)] Body Part", sortList(limb_list))
		if(result)
	...
	if(href_list[VV_HK_MARTIAL_ART])
		...
		var/result = tgui_input_list(usr, "Choose the martial art to teach","JUDO CHOP", sortList(artnames, /proc/cmp_typepaths_asc))
		if(!usr)
			return
		if(QDELETED(src))
	...
	if(href_list[VV_HK_GIVE_TRAUMA])
		...
		var/result = tgui_input_list(usr, "Choose the brain trauma to apply","Traumatize", sortList(traumas, /proc/cmp_typepaths_asc))
		if(!usr)
			return
		if(QDELETED(src))
	...
	if(href_list[VV_HK_HALLUCINATION])
		...
		var/result = tgui_input_list(usr, "Choose the hallucination to apply","Send Hallucination", sortList(hallucinations, /proc/cmp_typepaths_asc))
		if(!usr)
			return
		if(QDELETED(src))
			to_chat(usr, "Mob doesn't exist anymore")
