...
/mob/living/silicon/ai/proc/ai_network_change()
	...
	network = tgui_input_list(U, "Which network would you like to view?", buttons = sortList(cameralist))
...
/mob/living/silicon/ai/proc/ai_hologram_change()
	...
	switch(tgui_alert(usr,"Would you like to select a hologram based on a custom character, an animal, or switch to a unique avatar?",,list("Custom Character","Unique","Animal")))
			...
			switch(tgui_alert(usr,"Would you like to base it off of your current character loadout, or a member on station?",,list("My Character","Station Member")))
				if("Station Member")
					...
					if(personnel_list.len)
						input = tgui_input_list(usr, "Select a crew member:", buttons = sortList(personnel_list))
		...
		if("Animal")
			...
			input = tgui_input_list(usr, "Please select a hologram:", buttons = sortList(icon_list))
			if(input)
				qdel(holo_icon)
				switch(input)
					if("poly")
...
/mob/living/silicon/ai/verb/deploy_to_shell(mob/living/silicon/robot/target)
	...
	if(!target || !(target in possible)) //If the AI is looking for a new shell, or its pre-selected shell is no longer valid
		target = tgui_input_list(usr, "Which body to control?", buttons = sortNames(possible))
...
/mob/living/silicon/ai/verb/ai_cryo()
	...
	if(incapacitated())
		return
	if(tgalert(usr, "Would you like to enter cryo? This will ghost you. Remember to AHELP before cryoing out of important roles, even with no admins online.", "Enter Cryo?", "Yes", "No") != "Yes")
		return
	...
	if(GLOB.announcement_systems.len)
		...
		announcer.announce("CRYOSTORAGE_AI", src.real_name, announce_rank, list())
	new /obj/structure/ai_core/latejoin_inactive(loc)
	if(src.mind)
		//Handle job slot/tater cleanup.
		if(is_ai_job(src.mind.assigned_role))
			SSjob.FreeRole("AI")
